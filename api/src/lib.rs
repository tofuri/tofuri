...
fn parse_body(buffer: String) -> Result<String, Box<dyn Error>> {
    Ok(buffer.as_bytes().lines().last().ok_or("empty body")??)
}
pub mod get {
    use super::*;
    use pea_core::types;
    ...
    pub async fn info(api: &str) -> Result<types::api::Info, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/info", None).await?)?)?)
    }
    pub async fn sync(api: &str) -> Result<types::api::Sync, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/sync", None).await?)?)?)
    }
    pub async fn height(api: &str) -> Result<types::api::Height, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/height", None).await?)?)?)
    }
    pub async fn balance(api: &str, address: &str) -> Result<types::api::Amount, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(
            request(api, Method::GET, &format!("/balance/{}", address), None).await?,
        )?)?)
    }
    ...
    pub async fn hash(api: &str, height: &usize) -> Result<types::api::Hash, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(
            request(api, Method::GET, &format!("/hash/{}", height), None).await?,
        )?)?)
    }
    pub async fn dynamic(api: &str) -> Result<types::api::Dynamic, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/dynamic", None).await?)?)?)
    }
    pub async fn trusted(api: &str) -> Result<types::api::Trusted, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/trusted", None).await?)?)?)
    }
    pub async fn options(api: &str) -> Result<types::api::Options, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/options", None).await?)?)?)
    }
    pub async fn block(api: &str, hash: &str) -> Result<types::api::Block, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(
            request(api, Method::GET, &format!("/block/{}", hash), None).await?,
        )?)?)
    }
    pub async fn latest_block(api: &str) -> Result<types::api::Block, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(request(api, Method::GET, "/block/latest", None).await?)?)?)
    }
    ...
    pub async fn stake(api: &str, hash: &str) -> Result<types::api::Stake, Box<dyn Error>> {
        Ok(serde_json::from_str(&parse_body(
            request(api, Method::GET, &format!("/stake/{}", hash), None).await?,
        )?)?)
    }
}
pub mod post {
