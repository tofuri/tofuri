...
async fn balance(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::Balance(address_bytes)).await)
}
async fn balance_pending_min(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::BalancePendingMin(address_bytes)).await)
}
async fn balance_pending_max(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::BalancePendingMax(address_bytes)).await)
}
async fn staked(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::Staked(address_bytes)).await)
}
async fn staked_pending_min(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::StakedPendingMin(address_bytes)).await)
}
async fn staked_pending_max(State(s): State<Client>, address: Path<String>) -> impl IntoResponse {
    let address_bytes = public::decode(&address).unwrap();
    Json(s.call::<u128>(Call::StakedPendingMax(address_bytes)).await)
}
async fn height(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::Height).await)
}
async fn height_by_hash(State(s): State<Client>, hash: Path<String>) -> impl IntoResponse {
    let hash: [u8; 32] = hex::decode(hash.clone()).unwrap().try_into().unwrap();
    Json(s.call::<usize>(Call::HeightByHash(hash)).await)
}
async fn block_latest(State(s): State<Client>) -> impl IntoResponse {
    let block = s.call::<Block>(Call::BlockLatest).await;
    let block_hex: BlockHex = block.try_into().unwrap();
    Json(block_hex)
}
async fn hash_by_height(State(s): State<Client>, height: Path<String>) -> impl IntoResponse {
    let height: usize = height.parse().unwrap();
    let hash = s.call::<[u8; 32]>(Call::HashByHeight(height)).await;
    let hash_hex = hex::encode(hash);
    Json(hash_hex)
}
async fn block_by_hash(State(s): State<Client>, hash: Path<String>) -> impl IntoResponse {
    let hash: [u8; 32] = hex::decode(hash.clone()).unwrap().try_into().unwrap();
    let block = s.call::<Block>(Call::BlockByHash(hash)).await;
    let block_hex: BlockHex = block.try_into().unwrap();
    Json(block_hex)
}
async fn transaction_by_hash(State(s): State<Client>, hash: Path<String>) -> impl IntoResponse {
    let hash: [u8; 32] = hex::decode(hash.clone()).unwrap().try_into().unwrap();
...
async fn stake_by_hash(State(s): State<Client>, hash: Path<String>) -> impl IntoResponse {
    let hash: [u8; 32] = hex::decode(hash.clone()).unwrap().try_into().unwrap();
    let stake = s.call::<Stake>(Call::StakeByHash(hash)).await;
    let stake_hex: StakeHex = stake.try_into().unwrap();
    Json(stake_hex)
}
async fn peers(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<Vec<IpAddr>>(Call::Peers).await)
}
async fn peer(State(s): State<Client>, Path(ip_addr): Path<String>) -> impl IntoResponse {
    let ip_addr = ip_addr.parse().unwrap();
    Json(s.call::<bool>(Call::Peer(ip_addr)).await)
}
async fn transaction(
    State(s): State<Client>,
...
async fn stake(State(s): State<Client>, Json(stake): Json<StakeHex>) -> impl IntoResponse {
    let stake: Stake = stake.try_into().unwrap();
    Json(s.call::<bool>(Call::Stake(stake)).await)
}
async fn address(State(s): State<Client>) -> impl IntoResponse {
    Json(public::encode(&s.call::<[u8; 20]>(Call::Address).await))
}
async fn ticks(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::Ticks).await)
}
async fn time() -> impl IntoResponse {
    Json(chrono::offset::Utc::now().timestamp_millis())
}
async fn tree_size(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::TreeSize).await)
}
async fn sync(State(s): State<Client>) -> impl IntoResponse {
    let sync = s.call::<Sync>(Call::Sync).await;
    Json(sync)
}
async fn random_queue(State(s): State<Client>) -> impl IntoResponse {
    Json(
...
async fn unstable_hashes(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::UnstableHashes).await)
}
async fn unstable_latest_hashes(State(s): State<Client>) -> impl IntoResponse {
    Json(
...
async fn unstable_stakers(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::UnstableStakers).await)
}
async fn stable_hashes(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::StableHashes).await)
}
async fn stable_latest_hashes(State(s): State<Client>) -> impl IntoResponse {
    Json(
...
async fn stable_stakers(State(s): State<Client>) -> impl IntoResponse {
    Json(s.call::<usize>(Call::StableStakers).await)
}
async fn sync_remaining(State(s): State<Client>) -> impl IntoResponse {
    let sync = s.call::<Sync>(Call::Sync).await;
    if sync.completed {
        return Json(0.0);
    }
