...
pub fn balances(db: &DBWithThreadMode<SingleThreaded>) -> &ColumnFamily {
    db.cf_handle("balances").unwrap()
}
pub fn staked_balances(db: &DBWithThreadMode<SingleThreaded>) -> &ColumnFamily {
    db.cf_handle("staked_balances").unwrap()
}
