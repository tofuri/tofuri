...
lazy_static! {
    ...
    static ref TRANSACTION_SERIALIZED: usize = hex::encode(bincode::serialize(&Transaction::new([0; 32], 0, 0).unwrap()).unwrap()).len();
    static ref STAKE: Regex = Regex::new(r" /stake ").unwrap();
    static ref STAKE_SERIALIZED: usize = hex::encode(bincode::serialize(&Stake::new(false, 0, 0).unwrap()).unwrap()).len();
