...
pub async fn height_by_hash(addr: &str, hash: &Hash) -> Result<u128, Box<dyn Error>> {
...
pub async fn hash_by_height(addr: &str, height: &usize) -> Result<BlockA, Box<dyn Error>> {
    Ok(bincode::deserialize(&r(Data::HashByHeight, addr, Some(bincode::serialize(height)?)).await?)?)
}
pub async fn block_by_hash(addr: &str, hash: &Hash) -> Result<u128, Box<dyn Error>> {
    Ok(bincode::deserialize(&r(Data::BlockByHash, addr, Some(bincode::serialize(hash)?)).await?)?)
}
pub async fn transaction_by_hash(addr: &str, hash: &Hash) -> Result<u128, Box<dyn Error>> {
    Ok(bincode::deserialize(&r(Data::TransactionByHash, addr, Some(bincode::serialize(hash)?)).await?)?)
}
pub async fn stake_by_hash(addr: &str, hash: &Hash) -> Result<u128, Box<dyn Error>> {
    Ok(bincode::deserialize(&r(Data::StakeByHash, addr, Some(bincode::serialize(hash)?)).await?)?)
}
pub async fn peers(addr: &str) -> Result<u128, Box<dyn Error>> {
